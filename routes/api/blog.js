...
ipfsadd = async (
  ...
  try {
    ...
    var thumb = "https://troo.me/thumbs/" + thumbnailname;
    const address = await geocoder.reverse({
      lat: gpsoutput.latitude,
      lon: gpsoutput.longitude,
    });
    ...
    if (!blog) {
      const newBlog = new Blog({
        ...
        address:
          address[0].administrativeLevels.level2long +
          ", " +
          address[0].country,
...
router.post("/", auth, async (req, res, next) => {
  ...
  mediafile.mv(`${dirname}/public/${req.files.file.name}`, async (err) => {
    ...
    if (mediafile.mimetype.indexOf("video") !== -1) {
      ffmpeg.ffprobe(`${dirname}/public/${req.files.file.name}`, async function(
        err,
        metadata
      ) {
        ...
        if (meta == undefined || model == undefined || createdate == undefined)
      ...
      if (gpsoutput == undefined) return res.send("invalid");
        ...
        var pixstatus = await imagemark.checkcolumn(
          `${dirname}/public/${req.files.file.name}`
        );
  ...
  try {
    const skip =
      req.query.skip && /^\d+$/.test(req.query.skip)
        ...
        : 0;
    let skey;
    if (req.query.tag) skey = "#" + req.query.tag;
    else skey = req.query.skey ? req.query.skey : "";
    ...
    if (skey.search("@") == 0) {
      ...
      blogs = await Blog.find({ creator: { $in: creators } })
        .skip(skip)
        .sort({ date: -1 })
        .limit(5);
    } else
      blogs = await Blog.find({ description: { $regex: skey } })
        .skip(skip)
        .sort({ date: -1 })
        .limit(5);
    res.json(blogs);
...
router.post("/myfeed", auth, async (req, res) => {
  ...
  try {
    const blogs = await Blog.find(
      {
        description: { $regex: skey },
        saveusers: { $elemMatch: { user: req.user.id } },
      },
      undefined,
      { skip, limit: 5 }
    ).sort({ date: -1 });
...
router.post("/:id", auth, checkObjectId("id"), async (req, res) => {
  try {
    const blogs = await Blog.findOneAndDelete({ _id: req.params.id }).sort({
      date: -1,
    });
    res.json(blogs);
  } catch (err) {
...
router.post("/myfeed/:type/", auth, async (req, res) => {
  const AlluserInfo = await User.findOne({ _id: req.user.id });
  let userId = "";
    ...
    switch (type) {
      case "picture":
        blogs = await Blog.find(
          { filetype: "image", creator: userId, description: { $regex: skey } },
          undefined,
          { skip }
        ).sort({ date: -1 });
        break;
      ...
      case "earn":
        ...
        break;
      case "video":
        blogs = await Blog.find({ filetype: "video", creator: userId }).sort({
          date: -1,
        });
        break;
      case "all":
        blogs = await Blog.find(
          { creator: userId, description: { $regex: skey } },
          undefined,
          { skip }
        ).sort({ date: -1 });
        break;
      ...
      default:
        blogs = await Blog.find(
          { filetype: "video", creator: userId, description: { $regex: skey } },
          undefined,
          { skip }
        ).sort({ date: -1 });
    }
...
router.post("/byhashtag", auth, async (req, res) => {
  try {
    const blogs = await Blog.find({
      description: { $regex: req.body.hashtag },
    }).sort({ date: -1 });
    res.json(blogs);
  } catch (err) {
    console.error(err.message);
...
router.put("/donate", auth, async (req, res) => {
  ...
  User.findByIdAndUpdate(
    req.body.creatorid,
    { vcoin: Math.round(plustcoin * 100) / 100 },
    function(err, result) {
      if (err) {
        console.log(err);
      } else {
        console.log("Successfully sent to " + creator.pname);
      }
    }
  );
  User.findByIdAndUpdate(
    req.body.userid,
    { vcoin: Math.round(newvalue * 100) / 100 },
    function(err, result) {
      if (err) {
        res.send(err);
      } else {
...
router.put("/saveuser/:id", auth, checkObjectId("id"), async (req, res) => {
  try {
    ...
    if (
      blog.saveusers.some(
        (saveuser) => saveuser.user.toString() === req.user.id
      )
    ) {
      // remove the save
...
router.put("/report/:id", auth, checkObjectId("id"), async (req, res) => {
  try {
    ...
    if (
      !blog.reporters.some(
        (reporter) => reporter.user.toString() === req.user.id
      )
    )
      // remove the save

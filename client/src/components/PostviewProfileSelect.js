    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            const setblogs = async () => {
                setSkip(0);
                const res = await api.post('/profile/viewProfile', { select_name: props.userID });
                setMyprofile(res.data);
                const res1 = await api.post(`/blog/myfeed/${type}?skip=${skip}&skey=${searchword}`, { select_id: props.userID, flag: "viewUserProfile" });
                console.log(123, res1.data)
